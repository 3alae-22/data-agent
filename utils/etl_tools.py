...
class ETLTools:
    ...
    def execute_code(self, code: str, output_subdir: str | None = None):
        """
        Executes LLM-generated code in an isolated Python container
        (instead of a local exec(); see sandbox_executor.SandboxExecutor).

        output_subdir: if given, output files land in this fixed,
        predictable subfolder of data/outputs instead of a fresh
        random one each run (e.g. "plots" for visualizer_tool).
        """
        return self.sandbox.run(code, output_subdir=output_subdir)
